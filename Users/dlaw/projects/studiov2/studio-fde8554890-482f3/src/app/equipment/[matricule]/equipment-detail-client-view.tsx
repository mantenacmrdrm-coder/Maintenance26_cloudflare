...
export function EquipmentDetailClientView({ equipment, operations, preventativeHistory, curativeHistory, dynamicStatus }: ClientViewProps) {
  ...
  const lastCounterValue = lastCounter !== null ? parseFloat(String(lastCounter).replace(/\D/g, '')) : null;
  const newCounterValue = newCounter ? parseFloat(newCounter) : 0;
  const counterDifference = (newCounterValue > 0 && lastCounterValue !== null && newCounterValue > lastCounterValue)
                        ...
                        {counterDifference !== null && (
                            <div className="pt-1">
                                ...
                                <p className="text-xl font-bold text-primary">
                                    {counterDifference.toLocaleString('fr-FR')} {equipment.categorie?.toLowerCase().includes('camion') ? 'km' : 'heures'}
                                </p>
                            </div>
                        )}
                         {newCounterValue > 0 && lastCounterValue !== null && newCounterValue <= lastCounterValue && (
